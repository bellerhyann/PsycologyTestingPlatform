...
<script type = "text/javascript">
 ...
 if($data->num_rows > 0)
 {
     ...
     while($row = mysqli_fetch_array($data))
     {
        ...
        $queryString = ("SELECT * FROM trial_T WHERE trialID = $trialID");
        $trial = mysqli_query($conn, $queryString);
        $trialRow = mysqli_fetch_array($trial);
        ...
        if($trialRow['isCorrect'] == 1 && $row['clicked'] == 1 && $row['clickTime'] < 8000)
        {
            $sumCorrect++;
        }//if above is true, they clicked when they were supposed to. if below, they didn't click and weren't supposed to.
        else if($trialRow['isCorrect'] == 0 && ($row['clicked'] == 0 || $row['clickTime'] >= 8000))
        {
            $sumCorrect++;
        }
        $numTrials++;
